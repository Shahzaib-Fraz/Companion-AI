...
P0-11 FIX: parse_timezone() validates against the real IANA tz database.
P1-17 FIX: generate_verification_code() supports the two-phase WhatsApp
ownership verification flow in chat_service.py.
"""
...
class OnboardingService:
    ...
    async def parse_timezone(self, message: str) -> Optional[str]:
        ...
        P0-11 FIX: this used to accept ANY string containing "/" as a
        valid timezone with zero validation. Every candidate is now
        checked against the real tz database via validate_timezone()
        before being accepted; an invalid candidate returns None, same
        as every other "I didn't understand that" case in this class.
        """
        ...
        if "/" in msg:
            candidate = msg  # let validate_timezone be the judge
        else:
        ...
        try:
            return validate_timezone(candidate)  # canonical IANA form
        except TimezoneValidationError:
            logger.info(f"Rejected invalid timezone candidate during onboarding: {candidate!r}")
            return None
    ...
    async def parse_whatsapp_phone(self, message: str, db=None) -> Optional[str]:
        ...
        Note: a "valid" phone from this method is NOT yet saved anywhere.
        P1-17 FIX: chat_service.py now routes it through a WhatsApp
        ownership-verification step (see generate_verification_code()
        below) before it ever reaches User.phone_number.
        """
        msg = message.lower().strip()
    ...
    def generate_verification_code() -> str:
        """
        P1-17 FIX: 6-digit numeric code used to prove the person typing
        in this chat actually controls the WhatsApp number they entered,
        before it's saved to User.phone_number and used for outbound
        notifications. Uses `secrets` (not `random`) since this is
        standing in for a short-lived auth credential, however low-value.
        """
